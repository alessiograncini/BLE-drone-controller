...
// FIND THIS NUMBER UNDER BATTERY
const SERIAL_NUMBER = ""; //EX: "09700116375446"
//LOOK FOR THIS ADDRESS IN CONSOLE LOGS AFTER IT CONNECTS FOR THE FIRST TIME
const DEVICE_ADDRESS = new Uint8Array([0]); //EX: new Uint8Array([172, 142, 189, 82, 235, 129])
...
export class GameController {
 ...
 async scanForControllers() {
   ...
   await bluetoothModule.startScan(
     ...
     this.scanSetting,
     (result) => {
      this.log("Found: " + result.deviceName + " : " + result.deviceAddress);
      if (DEVICE_ADDRESS.length > 1) {
        if (this.addressesMatch(result.deviceAddress, DEVICE_ADDRESS)) {
  ...
  private addressesMatch(a: Uint8Array, b: Uint8Array): boolean {
 ...
 private async connectGATT(scanResult: Bluetooth.ScanResult) {
   ...
   gatt.onConnectionStateChangedEvent.add(async (connectionState) => {
    ...
    //use serial number to get mac address
    const infoService = gatt.getService("0x180A");
    const char = infoService.getCharacteristic("0x2A25");
    const serialNumber = Utf8.decode(await char.readValue());
